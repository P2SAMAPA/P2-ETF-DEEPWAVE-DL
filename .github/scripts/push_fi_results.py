...
- Overwrites today's file only: results/fi_{year}_{YYYYMMDD}.json
...
def main():
    ...
    today   = today_tag()
    year    = args.year
    target  = f"results/fi_{year}_{today}.json"

    # Delete today's existing file for this year (overwrite semantics)
    try:
    ...
    # FI model weights (no _eq files)
    weight_files = [
        f for f in (
            glob.glob(os.path.join(config.MODELS_DIR, "model_a", "**", "*.keras"), recursive=True) +
            glob.glob(os.path.join(config.MODELS_DIR, "model_b", "**", "*.keras"), recursive=True) +
            glob.glob(os.path.join(config.MODELS_DIR, "model_c", "**", "*.keras"), recursive=True) +
    ...
    print(f"  Pushing {len(ops)} files for fi year={year}")
    batched_commit(api, repo_id, ops, f"[auto] FI results year={year} {today}")
    print(f"  Done — fi_{year}_{today}.json → HF")
