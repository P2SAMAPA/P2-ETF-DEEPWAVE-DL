...
# Consensus fields stored as PERCENTAGES (ann_return=12.5 means 12.5%)
...
def result_path(year):
    ...
    return os.path.join(RESULTS_DIR, f"eq_{year}_{today_tag()}.json")
...
def load_equity_model(tag, lookback, wavelet):
...
def run_evaluation(start_year: int):
    ...
    if valid:
        ...
        results.update(dict(
            ...
            consensus_ann_return  = w["metrics"]["ann_return"],  # in %
            consensus_sharpe      = w["metrics"]["sharpe"],
            consensus_max_dd      = w["metrics"]["max_drawdown"],  # in %
        ))
