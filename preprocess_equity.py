...
def normalize_etf_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in df.columns:
        for etf in ETFS + config.BENCHMARKS:
...
def build_features(data: dict,
    ...
    etf_cols = [c for c in ETFS if c in etf_ret.columns]
    if not etf_cols:
        print(f"  WARNING: No equity ETF cols found. Available: {list(etf_ret.columns)}")

    etf_ret = etf_ret[etf_cols] if etf_cols else etf_ret
    etf_vol = etf_vol[[c for c in ETFS if c in etf_vol.columns]]
