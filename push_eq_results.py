...
- Overwrites today's file: results/eq_{year}_{YYYYMMDD}.json
...
def main():
    ...
    target_path = f"results/eq_{year}_{today}.json"
    try:
    ...
    # ── Push equity model weights (batched) ───────────────────────────────────
    weight_files = [
        f for f in (
    ...
    print(f"  Pushing {len(all_ops)} files for eq year={year}")
    batched_commit(api, repo_id, all_ops,
                   commit_message=f"[auto] Equity results year={year} {today}")
    print(f"  Done — eq_{year}_{today}.json pushed to HF")
